...
module.exports = Router()
	.post('/', async (req, res) => {
		const tea = await addTea({
			...
			harvestYear: req.body.harvestYear ?? 'unknown',
			origin: req.body.origin ?? 'unknown',
			cultivar: req.body.cultivar ?? 'unknown',
			elevation: req.body.elevation ?? 'unknown',
		});
		...
		const tea = await updateTea(
			{
				...
				...(req.body.vendorName && { vendorName: req.body.vendorName }),
				...(req.body.teaType && { teaType: req.body.teaType }),
				...(req.body.inStock && { inStock: req.body.inStock }),
				...(req.body.quantity && { quantity: req.body.quantity }),
				...(req.body.harvesYear && { harvestYear: req.body.harvesYear }),
